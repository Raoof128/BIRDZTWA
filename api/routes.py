...
from .models import (
    RenderRequest, RenderResponse, RenderStatus,
    PolicyCheckRequest, PolicyCheckResponse,
    AuditLogResponse, HealthResponse, PolicySummaryResponse,
    ErrorResponse
)
...
async def render_url(request: RenderRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if not policy_decision.allowed:
            ...
            raise HTTPException(
                status_code=403,
                detail=f"URL blocked by policy: {policy_decision.reason}"
            )
        
        # Update stats
        stats["active_renders"] += 1
        ...
        sanitizer = DOMSanitizer(
            remove_trackers=request.block_trackers
        )
